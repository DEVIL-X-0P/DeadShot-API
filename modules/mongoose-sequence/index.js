...
module.exports = function SequenceFactory(connection) {
     ...
     *                                  constructor
     * @return     {Index}  A sequence
     *
     * @static
     */
    Index.getInstance = function (schema, options) {
        const sequence = new Index(schema, options);
     ...
     *                       reference
     */
    Index.prototype._getCounterReferenceField = function (doc) {
        let reference = {};
     ...
     */
    Index.prototype._createSchemaKeys = function () {
     ...
     */
    Index.prototype._getPreSaveHook = function () {
        const sequence = this;
     ...
     * @method     _setHooks
     */
    Index.prototype._setHooks = function () {
     ...
     * @method     _setMethods
     */
    Index.prototype._setMethods = function () {
    ...
    Index.prototype._resetCounter = function (id, reference, callback) {
        const condition = { id };
        let cb = callback;
        let seq = 0;
        ...
        getModel(Models.Counter).updateMany(condition, { $set: { seq } }, null, cb);
    };
     ...
     */
    Index.prototype._setNextCounter = function (doc, callback) {
        const retriable = (cb) => {
            const id = this.getId();
            const referenceValue = this._getCounterReferenceField(doc);
            const incAmount = this._options.inc_amount;
            getModel(Models.Counter).findOneAndUpdate(
                { id, reference_value: referenceValue, tenant:ObjectId(doc.tenant) },
                { $inc: { seq: incAmount } },
     ...
     * @param      {Function}  callback  Called with the sequence counter
     */
    Index.prototype._createCounter = function (doc, callback) {
        const id = this.getId();
        const referenceValue = this._getCounterReferenceField(doc);
        const startSeq = this._options.start_seq;
        const counterModel = getModel(Models.Counter);

        counterModel.findOneAndUpdate(
            {
                id,
                reference_value: referenceValue,
                tenant:ObjectId(doc.tenant)
            },
            {},
            {
                upsert: true,
